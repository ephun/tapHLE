...
- Route: `dev-docs/clickmaps/cut-the-rope.json`. No launch options.
...
day; the boundary is recorded in `compatibility/UNFILED.md` with the others.
Both this boundary and the two-star one it passed on the way are unfiled, and
the second cannot be reconstructed later.
