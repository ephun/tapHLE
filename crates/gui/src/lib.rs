...
pub extern "C" fn SDL_main(
    ...
    match run() {
        Ok(()) => 0,
        Err(error) => {
            sdl2::log::log(&format!("tapHLE frontend error: {error}"));
            1
        }
    }
}
...
/// Enter SDL's UIKit lifecycle from the process-level iOS `main` function.
#[cfg(target_os = "ios")]
#[no_mangle]
pub extern "C" fn tapHLE_iOS_main(
    argc: std::ffi::c_int,
    argv: *mut *mut std::ffi::c_char,
) -> std::ffi::c_int {
